...
//    }
    ...
    @ResponseBody
    public String loadPageFragment(@PathVariable String name) {
        return "page/" + name + " :: contentFragment";
    }
    @GetMapping("/intro/{name}")
    public String showPage(@PathVariable String name, Model model) {
        model.addAttribute("pageTitle", "사업제휴문의");
        // layout.html을 사용하고, 초기 main 영역에 fragment 삽입
        model.addAttribute("contentFragment", "page/" + name + " :: contentFragment");
        return "layout";
    }
}
